...
# Logo image path (place trades_gpt_logo.png in same folder as bot)
...
def get_admin_approval_keyboard(user_id: int):
    """Admin approval keyboard for deposit proof"""
    keyboard = [
        [
...
def generate_binary_signal():
    """Generate a binary options trading signal with timing"""
    pair = random.choice(ALLOWED_PAIRS)
    signal_type = random.choice(SIGNAL_TYPES)

    # Weighted timeframe selection (5M most common for binary)
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /start command - Show Trade GPT welcome message with logo"""
    ...
    # Send logo image with caption as welcome message
    try:
...
async def signal_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /signal command - Generate binary trading signal"""
...
async def handle_proof_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle deposit proof photo submission"""
    user_id = update.effective_user.id
...
def main():
    ...
    print("🚀 Trades GPT Bot started! Press Ctrl+C to stop.")
